...
use crate::types::{
    AnalysisTemplate, KnowledgeIndexStatus, KnowledgeSettings, McpAccessLog, McpStatus,
    ProcessingJob, Project, RecordBrief, SearchResult, StoredAnalysis, TemplateSection,
    TranscriptBlock, TranscriptSegment, TranscriptSegmentInput, TranscriptVersion,
};
use chrono::Utc;
...
impl LibraryRepository {
    ...
    pub fn create_record(
        ...
        self.connect()?.execute(
            "INSERT INTO records \
             (id, title, project_id, audio_path, audio_hash, audio_duration_ms, imported_at, processing_status, created_at, updated_at) \
             VALUES (?1, ?2, NULLIF(?3, ''), ?4, ?5, ?6, ?7, ?8, ?7, ?7)",
            params![
                record.id,
                record.title,
                project_id,
    ...
    pub fn search(
        ...
        let statement_sql = if query.chars().count() < 3 {
            &short_sql
        } else {
            &sql
        };
        ...
        let query_param = if query.chars().count() < 3 {
            query
        } else {
            fts_query.as_str()
        };
