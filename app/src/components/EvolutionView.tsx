...
function ConfiguredEmpty({ onOpenSettings }: { onOpenSettings: () => void }) {
  return <div className="memory-config-empty"><div className="memory-config-icon"><SparklesIcon size={22} /></div><h3>认知演化需要外部 AI</h3><p>启用后，模型只分析你确认发送的文本，不包含原始音频。</p><button type="button" className="primary-button" onClick={onOpenSettings}>打开外部 AI 设置</button></div>;
}
